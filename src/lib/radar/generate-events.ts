  ...
  for (const asset of assets) {
    ...
    if ((asset.score ?? 0) >= 80) {
      events.push({
        ...
        message: `IQ-Score de ${asset.score} — fundamentos sólidos destacam-se no setor`,
    ...
    if (portfolioTickers.includes(asset.ticker) && (asset.score ?? 0) >= 60 && (asset.score ?? 0) < 80) {
      events.push({
        ...
        message: `IQ-Score de ${asset.score} — ativo da sua carteira com fundamentos saudáveis`,
